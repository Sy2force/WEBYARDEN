...
export default function ServiceCard({ service, index = 0 }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group card-service overflow-hidden hover:border-indigo-200 dark:bg-gray-800 dark:border-gray-700 dark:hover:border-indigo-600"
    >
      <Link href={`/services/${service.slug}`} className="block">
        ...
        <div className="p-6">
          ...
          <ul className="space-y-3 mb-6">
            {t(service.featuresKey).split(',').slice(0, 3).map((feature: string, featureIndex: number) => (
              <li key={featureIndex} className="flex items-center group/feature">
                ...
                <span className="text-gray-700 text-sm font-medium group-hover/feature:text-gray-900 transition-colors duration-200">{feature.trim()}</span>
              </li>
            ))}
          </ul>
