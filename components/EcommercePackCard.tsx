...
interface EcommercePackCardProps {
  pack: Pack
  index?: number
...
export default function EcommercePackCard({ pack, index = 0, isHorizontal = false, isCube = false }: EcommercePackCardProps) {
      ...
      >
        ...
        {(pack.badge || pack.popular) && (
          <div className="absolute top-2 left-2 z-10">
            <span className={`${pack.popular ? 'bg-gradient-to-r from-indigo-600 to-purple-600' : pack.badgeColor || pack.color} text-white px-2 py-1 rounded-lg text-xs font-bold shadow-lg`}>
        ...
        <div className="relative p-4 h-full flex flex-col">
          ...
          <div className="text-center mb-3">
            {pack.originalPrice && (
              <div className="text-xs text-gray-400 line-through mb-1">
                {pack.currency}{pack.originalPrice.toLocaleString()}
              </div>
            )}
            <div className="text-2xl font-black bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
              {pack.price > 0 ? `${pack.currency}${pack.price.toLocaleString()}` : t('packs.onQuote')}
            </div>
          </div>
          ...
          <div className="flex-1 mb-3">
            <ul className="space-y-1 text-xs text-gray-600 dark:text-gray-400">
              {pack.services.slice(0, 3).map((service: string, idx: number) => (
      ...
      >
        ...
        {pack.badge && !pack.popular && (
          <div className="absolute -top-3 -right-3 z-10">
            <span className={`${pack.badgeColor || pack.color} text-white px-3 py-1 rounded-full text-xs font-bold shadow-xl transform rotate-12`}>
              {pack.badge}
            </span>
          </div>
        )}
        ...
        <div className="relative p-6 h-full flex flex-col">
          ...
          <div className="flex-1 mb-4">
            <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
              {pack.services.slice(0, 4).map((service: string, idx: number) => (
                <li key={idx} className="flex items-center gap-2">
  ...
  return (
    ...
    >
      ...
      {pack.badge && !pack.popular && (
        <div className="absolute -top-3 -right-3 z-10">
          <span className={`${pack.badgeColor || pack.color} text-white px-4 py-2 rounded-full text-xs font-bold shadow-xl transform rotate-12`}>
            {pack.badge}
          </span>
        </div>
      )}
        ...
        <div className="text-center mb-8 bg-gray-50 dark:bg-gray-700/50 rounded-2xl p-6">
          {pack.originalPrice && (
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <span className="bg-red-100 text-red-600 px-2 py-1 rounded-full text-xs font-bold">
                {t('packs.save')} {pack.currency}{(pack.originalPrice - pack.price).toLocaleString()}
              </span>
            </div>
          )}
          
          <div className="flex items-baseline justify-center gap-1 mb-2">
            <span className="text-sm text-gray-500 dark:text-gray-400">{pack.currency}</span>
          ...
          <div className="space-y-1">
            <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">
              {t('common.delivery')}: {pack.delivery}
            </p>
          </div>
        </div>
